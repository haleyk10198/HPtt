...
*/
{
	T[id].value = max(lhs.value, rhs.value);
}
...
void apply(int id, data_t value)
{
	T[id].value = value;
}
...
void update(int position, data_t value, int id = 1, int l = 0, int r = n)
/*
	Update a[position]
...
*/
{
	...
	update(position, value, id<<1, l, l+r>>1); // Try to update with the left child T[id*2] which contains [l, mid)
	update(position, value, id<<1|1, l+r>>1, r); // Try to update with the right child T[id*2+1]...
...
int main(){
	...
	for(int i = 0; i < 10; i+=2)
		update(a[i], i*2); // a[i] *= 2 for even i
	
	for(int x, y; cin >> x >> y; ){
		cout << "The max of a[" << x << ", " << y << ") is: " << ask(x, y) << endl;
	}
}
